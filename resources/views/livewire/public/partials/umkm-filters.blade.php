...
<form
    ...
    data-defaults='{"category":"","rw":"","verified":"1","sort":"latest","perPage":"9"}'
...
>
    ...
    <div>
        ...
        <select id="category-filter-{{ $filterIdSuffix }}" name="category" class="mt-2 min-h-11 w-full rounded-input border border-cimuning-border bg-white px-4 text-base text-cimuning-charcoal focus:border-cimuning-red focus:outline-2">
    ...
    <div>
        ...
        <select id="rw-filter-{{ $filterIdSuffix }}" name="rw" class="mt-2 min-h-11 w-full rounded-input border border-cimuning-border bg-white px-4 text-base text-cimuning-charcoal focus:border-cimuning-red focus:outline-2">
            <option value="">Semua RW</option>
            @foreach ($rws as $item)
                <option value="{{ $item }}" @selected($rw === $item)>{{ $item }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div>
        ...
        <select id="sort-filter-{{ $filterIdSuffix }}" name="sort" class="mt-2 min-h-11 w-full rounded-input border border-cimuning-border bg-white px-4 text-base text-cimuning-charcoal focus:border-cimuning-red focus:outline-2">
    ...
    <div>
        ...
        <select id="per-page-filter-{{ $filterIdSuffix }}" name="perPage" class="mt-2 min-h-11 w-full rounded-input border border-cimuning-border bg-white px-4 text-base text-cimuning-charcoal focus:border-cimuning-red focus:outline-2">
            <option value="9" @selected((int) $perPage === 9)>9 UMKM</option>
            <option value="18" @selected((int) $perPage === 18)>18 UMKM</option>
            <option value="27" @selected((int) $perPage === 27)>27 UMKM</option>
        </select>
    </div>
    ...
    <div>
        ...
        <label class="mt-3 flex min-h-11 items-center gap-3 rounded-xl border border-cimuning-border bg-white px-3 text-base text-cimuning-charcoal">
            ...
            <input type="checkbox" name="verified" value="1" @checked($verified) class="h-5 w-5 rounded border-cimuning-border text-cimuning-green focus:ring-cimuning-red">
        ...
        <div class="mt-3 space-y-2">
            @foreach ([
                ...
                <label class="flex min-h-11 items-center gap-3 rounded-xl border border-cimuning-border bg-white px-3 text-base text-cimuning-charcoal">
                    <input type="checkbox" name="services[]" value="{{ $value }}" @checked(in_array($value, $activeServices, true)) class="h-5 w-5 rounded border-cimuning-border text-cimuning-red focus:ring-cimuning-red">
                    {{ $label }}
                </label>
            @endforeach
        </div>
