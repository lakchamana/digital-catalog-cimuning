...
- Admin memiliki halaman `Backup Data` dengan ringkasan jadwal, riwayat responsif, panduan penyimpanan sederhana, serta pemeriksaan file pemulihan yang disembunyikan sampai dibutuhkan. Password akun dan password backup wajib, password backup tidak disimpan, dan satu admin dibatasi satu permintaan per 15 menit.
...
- Media tidak masuk ZIP database. Backup Railway/Cloudinary bersifat opsional selama deployment hanya untuk testing tim; strategi backup penyedia hosting dan media wajib ditetapkan kembali saat hosting production final dipilih.
...
- Media upload dibatasi 2 MB, JPG/PNG/WEBP, dimensi 5000×5000, dan 20 request/menit; adapter juga memvalidasi path, ukuran, MIME signature, dan stream.
- Delivery Cloudinary memakai signed URL. Aktifkan Strict Transformations secara manual hanya setelah signed URL production terverifikasi.
- `media:diagnose` bersifat read-only; flag `--upload` menjalankan smoke upload dan cleanup otomatis tanpa menyimpan database record.
